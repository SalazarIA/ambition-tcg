...
def _node(
    ...
    *,
    ...
    return {
        ...
        "bot_difficulty_id": "easy" if order <= 2 else "hard" if order >= 7 else "normal",
        "bot_hp": bot_hp,
        "player_hp": 30,
        "reward": {"xp": xp},
        "unlock": unlock,
...
_CAMPAIGN_NODES = (
    _node(
        "node_01_acolyte", 1, "Acólito da Brasa",
        "Uma chama jovem guarda o primeiro portal. Vença para seguir.",
        "novice", 18, 60, None, ("card_001", "card_021", "card_041"),
        presentation=_presentation("O Primeiro Portal", "fire", "#e6ad4b", "ember"),
        loss_tip="Construa um monstro antes de buscar dano direto; o Acólito deixa janelas abertas.",
        key_card_id="card_001",
    ),
    _node(
        "node_02_guardian", 2, "Guardião de Pedra",
    ...
    _node(
        ...
        "O senhor das cinzas exige uma vitória sem hesitação.",
        ...
        loss_tip="Pyrelord vence corridas longas; troque cedo e não deixe seus PV virarem combustível.",
        key_card_id="card_004",
    ),
    _node(
        "node_04_stone_sentinel", 4, "Sentinela de Pedra",
        "Um colosso antigo fecha a trilha com runas de resistência.",
    ...
    _node(
        "node_05_tide_witch", 5, "Bruxa da Maré",
        "As águas sobem a cada decisão lenta; a bruxa joga com fôlego infinito.",
    ...
    _node(
        ...
        "O machado já está levantado. Cada turno entregue vira uma sentença.",
        "aggressive", 39, 210, "node_05_tide_witch", ("card_004", "card_003", "card_002", "card_001"),
        modifiers=[{"id": "opening_mana", "side": "bot", "amount": 1}],
        presentation=_presentation("Sentença Rubra", "fire", "#ff463f", "inferno", "heavy"),
        loss_tip="Defenda o primeiro pico; o Carrasco é mais vulnerável depois de gastar sua abertura.",
        key_card_id="card_004",
    ),
    _node(
        "node_07_eclipse_parasite", 7, "Parasita do Eclipse",
        "A sombra não ataca só o corpo: ela aprende com suas feridas.",
        ...
        loss_tip="Limpe deterioração quando puder e evite trocas que alimentem roubo de vida.",
    ...
    _node(
        ...
        loss_tip="Não desperdice ataque no escudo inicial; monte campo e derrube tudo em cadeia.",
        key_card_id="card_046",
    ),
    _node(
        ...
        presentation=_presentation("A Última Profecia", "shadow", "#d493ff", "void", "heavy"),
        loss_tip="O Arauto abre com opções demais; priorize sobrevivência até a mão dele estreitar.",
        key_card_id="card_065",
    ),
    _node(
        "node_10_gray_king", 10, "Rei Cinzento",
        "No trono frio, fogo, pedra e sombra obedecem a uma única coroa.",
        ...
        loss_tip="Quebre a coroa em fases: sobreviva à abertura, remova o escudo e só então acelere o dano.",
        key_card_id="card_004",
    ),
)
