...
def run_backend_flow():
    ...
    try:
        with app.app_context():
            ...
            match = create_training_match(user, "QA_BACKEND_SID", "qa_backend_room")
            ...
            playable_ids = after_intent["legal_actions"]["playable_card_ids"] or []
            first_card_id = next(
                (
                    card["id"]
                    for card in after_intent["me"]["hand"]
                    if card.get("type") == "Monster" and card["id"] in playable_ids
                ),
                playable_ids[0] if playable_ids else after_intent["me"]["hand"][0]["id"],
            )
            ...
            monster = after_play["me"]["field"].get("monster")
            if monster:
