...
STARTER_MONSTER_COUNT = int(STARTER_DECK_SIZE * STARTER_DECK_MONSTER_RATIO)
STARTER_SPELL_COUNT = int(STARTER_DECK_SIZE * STARTER_DECK_SPELL_RATIO)
STARTER_TRAP_COUNT = STARTER_DECK_SIZE - STARTER_MONSTER_COUNT - STARTER_SPELL_COUNT
...
def create_starter_collection():
    ...
    for element in ELEMENTS:
        ...
        common_amount = 14
        uncommon_amount = STARTER_COLLECTION_MONSTERS_PER_ELEMENT - common_amount

        collection_ids.extend(choose_unique_cards(common_pool, common_amount))
        collection_ids.extend(choose_unique_cards(uncommon_pool, uncommon_amount))
...
def create_starter_deck_from_collection(collection_ids):
    ...
    for index, element in enumerate(ELEMENTS):
        ...
        if len(pool) >= amount:
            ...
            deck_ids.extend(choose_cards_with_duplicates(pool, missing))
...
def validate_deck(deck_ids, collection_ids):
    ...
    if len(deck_ids) < DECK_RULES["min_cards"]:
        errors.append(f"Deck must have at least {DECK_RULES['min_cards']} cards.")

    if len(deck_ids) > DECK_RULES["max_cards"]:
        errors.append(f"Deck must have at most {DECK_RULES['max_cards']} cards.")
    ...
    if type_counts["Monster"] < DECK_RULES["min_monsters"]:
        errors.append(f"Deck must have at least {DECK_RULES['min_monsters']} monsters.")

    if type_counts["Monster"] > DECK_RULES["max_monsters"]:
        errors.append(f"Deck must have at most {DECK_RULES['max_monsters']} monsters.")

    if type_counts["Spell"] < DECK_RULES["min_spells"]:
        errors.append(f"Deck must have at least {DECK_RULES['min_spells']} spells.")

    if type_counts["Spell"] > DECK_RULES["max_spells"]:
        errors.append(f"Deck must have at most {DECK_RULES['max_spells']} spells.")

    if type_counts["Trap"] < DECK_RULES["min_traps"]:
        errors.append(f"Deck must have at least {DECK_RULES['min_traps']} traps.")

    if type_counts["Trap"] > DECK_RULES["max_traps"]:
        errors.append(f"Deck must have at most {DECK_RULES['max_traps']} traps.")
