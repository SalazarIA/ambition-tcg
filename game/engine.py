...
def can_unleash_ambition(player):
    return int(player.get("ambition", 0)) >= AMBITION_UNLEASH_COST


def unleash_ambition_if_possible(player, logs=None):
    if not can_unleash_ambition(player):
        return 0

    if not player.get("field_m"):
        return 0
    ...
    if logs is not None:
        logs.append(
            f"{player['name']} unleashed Ambition: {player['field_m']['name']} gains +{AMBITION_UNLEASH_POWER_BONUS} power this battle."
        )
