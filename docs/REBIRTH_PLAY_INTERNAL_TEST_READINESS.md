...
Data da reauditoria: 28 de junho de 2026<br>
...
|---|---|---|
| Produção Rebirth | Operacional com divergências | `ambitionzgame.com` entrega Arena, campanha, coleção, deck builder, loja, recompensas, ranking, perfil e suporte; o suporte expõe `v132_PVP_ROBUST`. |
...
| Privacidade/Termos/Exclusão | Públicas com copy local reconciliada | URLs respondem por HTTPS e a exclusão é encontrável. Os templates locais removem “beta fechada”/“Teste Fechado”; produção publicada precisa de novo deploy e revalidação. |
...
- Copy adotada: “armadilhas reativas e magias com alvo em unidades” e “PvP experimental”, sem prometer estabilidade competitiva, pareamento garantido ou controle manual de uma stack.
...
Auditoria HTTP reexecutada em 21 de junho de 2026 e documentação reconciliada em 28 de junho de 2026:
...
- A página de suporte mostra `Versão v132_PVP_ROBUST`.
...
- Privacidade, termos e exclusão foram reconciliados localmente para não exibir “Teste Fechado”/“beta fechada” nos textos públicos desta release. Como não houve deploy nesta auditoria, a produção publicada ainda precisa ser revalidada após promoção.

## PWA, cache e headers

- `ASSET_VERSION` permanece alinhada ao runtime web: `v132_PVP_ROBUST`.
...
- Release notes aceitam até 500 caracteres Unicode por idioma; o texto pt-BR preparado tem 435 caracteres sem a quebra final do arquivo.
...
- [x] Domínio, runtime v131 e páginas públicas auditados/reconciliados.
- [x] Textos/checklists migrados de closed/training legado para Internal/Rebirth.
- [x] Listing e release notes auditados contra Onda 1–3, PvP experimental e produção.
- [x] Evolução corrigida para monstros por duplicata; promessa de partida rápida removida.
- [x] Release notes pt-BR validadas com 435/500 caracteres sem a quebra final do arquivo.
...
- [x] Normalizar localmente “beta fechada”/“Teste Fechado” versus Internal testing nos templates legais.
- [ ] Revalidar em produção, após deploy, que os textos legais publicados já refletem a normalização local.
...
O AAB móvel beta.2 está pronto para ser apresentado ao Play Console como candidato de Internal testing, sujeito à confirmação de package e `versionCode 2` e às validações do upload. Listing, release notes e instruções agora estão coerentes com as Ondas 1–3, PvP experimental e produção, sem promessas de offline, duração curta, pareamento garantido ou contagem exata de cartas. Screenshots, ícone e feature graphic atendem aos formatos documentados; os dois últimos ainda exigem aprovação visual do owner e validação no Console. As pendências de deploy/revalidação dos textos legais, catálogo 100/103, HSTS e confirmação do cache `no-cache` em produção para `/service-worker.js` seguem registradas para os owners responsáveis.
