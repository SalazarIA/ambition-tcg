...
|---|---|---|
| Produção Rebirth | Operacional com divergências | `ambitionzgame.com` entrega Arena, campanha, coleção, deck builder, loja, recompensas, ranking, perfil e suporte; o suporte expõe `v119_ONBOARDING_FINALE`. |
...
- A página de suporte mostra `Versão v119_ONBOARDING_FINALE`.
...
- `ASSET_VERSION` permanece alinhada ao runtime web: `v119_ONBOARDING_FINALE`.
...
- `/static/js/service-worker.js` respondeu com `Cache-Control: public, max-age=2592000`, não com `no-cache`. Isso contradiz o contrato documental anterior e pode atrasar a percepção de uma troca de runtime.
