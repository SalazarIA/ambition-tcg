...
def admin_dashboard():
    ...
    for match in all_matches:
        ...
        for log_line in logs:
            ...
            for sigil_name in sigil_metrics.keys():
                if f"{sigil_name} Sigil" in line:
                    sigil_metrics[sigil_name] += 1
...
def feedback():
    ...
    if request.method == "POST":
        ...
        existing_feedback_count = FeedbackReport.query.filter_by(user_id=user.id).count()

        if existing_feedback_count >= app.config.get("FEEDBACK_DAILY_LIMIT", 10) * 100:
            flash("Feedback limit reached.")
            return redirect("/")
...
def resend_verification():
    if request.method == "POST":
        ...
        if sent:
            ...
            flash("SMTP is not configured. Verification link was printed in server logs.")
    ...
    with db.engine.begin() as connection:
        for table in tables_to_clear:
            try:
                ...
                print("Reset ignored:", table, error)

        connection.execute(
            sql_text("DELETE FROM users WHERE is_admin = false")
        )

    log_system_event("warning", "admin", "Non-admin users reset by admin", user_id=user.id)
    flash("Non-admin users and test data deleted.")
    ...
    with db.engine.begin() as connection:
        for table in tables_to_clear:
            try:
                ...
                print("Clear ignored:", table, error)

    log_system_event("warning", "admin", "Gameplay data cleared by admin", user_id=user.id)
    flash("Gameplay data cleared. Users were preserved.")
...
def admin_invites():
    ...
    if request.method == "POST":
        ...
        log_system_event("info", "admin", f"Beta invite created: {code}", user_id=user.id)
...
def admin_feedback_update(report_id):
    ...
    if report:
        ...
        db.session.commit()
        log_system_event("info", "feedback", f"Feedback status updated: {report.title}", user_id=user.id)
