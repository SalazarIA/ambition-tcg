...
def test_rebirth_service_worker_caches_active_reference_assets():
    ...
    assert 'const ASSET_VERSION = "v109_READABILITY";' in service_worker
    assert 'REBIRTH_RELEASE_VERSION = os.environ.get("REBIRTH_RELEASE_VERSION", "v109_READABILITY")' in app_runtime
    assert "`ASSET_VERSION` permanece alinhada ao runtime web: `v109_READABILITY`." in internal_testing_readiness
