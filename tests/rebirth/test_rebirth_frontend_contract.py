...
def test_rebirth_service_worker_caches_active_reference_assets():
    ...
    assert 'const ASSET_VERSION = "v115_PROFILE_AVATARS";' in service_worker
    assert 'REBIRTH_RELEASE_VERSION = os.environ.get("REBIRTH_RELEASE_VERSION", "v115_PROFILE_AVATARS")' in app_runtime
    assert "`ASSET_VERSION` permanece alinhada ao runtime web: `v115_PROFILE_AVATARS`." in internal_testing_readiness
