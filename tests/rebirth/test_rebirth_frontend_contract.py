...
def test_rebirth_service_worker_caches_active_reference_assets():
    ...
    assert 'const ASSET_VERSION = "v116_PROFILE_PICKER";' in service_worker
    assert 'REBIRTH_RELEASE_VERSION = os.environ.get("REBIRTH_RELEASE_VERSION", "v116_PROFILE_PICKER")' in app_runtime
    assert "`ASSET_VERSION` permanece alinhada ao runtime web: `v116_PROFILE_PICKER`." in internal_testing_readiness
