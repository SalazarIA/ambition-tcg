...
/* Ambitionz Web Polish: post-match clarity */
(function () {
  function enhancePostMatch(payload) {
    try {
      if (!payload || document.querySelector(".ambition-postmatch-cta")) return;

      const container =
        document.querySelector("#post-match-summary") ||
        document.querySelector(".post-match-summary") ||
        document.querySelector(".game-container") ||
        document.body;

      const box = document.createElement("div");
      box.className = "ambition-postmatch-cta post-match-card";
      box.innerHTML = `
        <h3>Match Complete</h3>
        <p>Your rewards and progression were updated. Check missions, open boosters or improve your deck.</p>
        <div class="landing-actions">
          <a class="landing-button primary" href="/missions">Claim Missions</a>
          <a class="landing-button secondary" href="/shop">Open Booster</a>
          <a class="landing-button ghost" href="/deck-builder">Edit Deck</a>
        </div>
      `;

      container.appendChild(box);
    } catch (error) {
      console.warn("Post-match CTA enhancement skipped:", error);
    }
  }

  document.addEventListener("DOMContentLoaded", function () {
    if (window.socket && typeof window.socket.on === "function") {
      window.socket.on("post_match_summary", enhancePostMatch);
      window.socket.on("game_over", enhancePostMatch);
    }
  });
})();
